...
d2 = 1 * f  # dis between lens1 and lens2
size = 1000  #
dx = 8e-6  #
sample_path = 'usaf.png'
if_obj = True
Add_zernike = False
phase_correct = False
SGD_correct = False
...
obj = creat_obj(sample_path, size, radius=400, binaty_inv=True, if_obj=if_obj)
...
zer = (zer % (2 * torch.pi)).to(device)  # 0-2pi
zer[mask] = 0.0
...
len1_phs = lens_phase(X, Y, k, f).to(device)  # lens1 phase
...
len2_phs = lens_phase(X, Y, k, f).to(device)  # lens1 phase
