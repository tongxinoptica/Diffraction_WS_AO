...
zer[mask] = 0.0
zer_phase = torch.exp(1j * zer)
# plt.imshow(zer.data.numpy(), cmap='gray')
# plt.show()
# plt.imsave('./test_img/zer_53_40.png', zer.data.numpy(), cmap='gray')

if Add_zernike:
    # Propagate abe to obj
    zer_phase = zer_phase.unsqueeze(0).unsqueeze(0)
    free_d0 = Diffraction_propagation(zer_phase, d0, dx, lambda_)
    free_d0_amp = get_amplitude(free_d0)
    free_d0_ph = get_phase(free_d0)
    obj_field = obj.unsqueeze(0).unsqueeze(0) * free_d0_amp * torch.exp(1j * free_d0_ph)
    # plt.imshow(free_d0_amp.squeeze(0).squeeze(0), cmap='gray')
    # plt.show()
else:
    obj_field = obj.unsqueeze(0).unsqueeze(0)

# Propagate free to lens1
free_d1 = Diffraction_propagation(obj_field, d1, dx, lambda_)
...
plt.show()
# plt.imsave('./test_img/obj_0.5f_zer_amp.png', free_d3_amp.squeeze(0).squeeze(0).data.numpy(),cmap='gray')
plt.imshow(free_d3_ph.squeeze(0).squeeze(0).data.numpy(), cmap='gray')
plt.show()
# imageio.imwrite('./test_img/obj_0.5f_nozer_ph.png', phasemap_8bit(free_d3_ph, False))
# plt.imsave('./test_img/obj_0.5f_zer_ph.png', free_d3_ph.squeeze(0).squeeze(0).numpy(), cmap='gray')

if phase_correct:
    d4 = 0.5 * f
    ph_zer = cv2.imread('test_img/obj_0.5f_zer_ph.png', cv2.IMREAD_GRAYSCALE)
    ph_nozer = cv2.imread('test_img/obj_0.5f_nozer_ph.png', cv2.IMREAD_GRAYSCALE)
    ...
    free_d4_field = free_d3_amp * torch.exp(1j * free_d3_ph)
