...
def calculate_slm_input(n, dx, d0, lambda_, device):
    ...
    for path in image_paths:
        ...
        img = img[430:2430, 1030:3030]
        # img = cv2.resize(img, (862, 862), interpolation=cv2.INTER_CUBIC)
        # img / img.max()
    ...
    recovery_slm_field = random_phase_recovery(intensity, rand_p, d0, dx, lambda_, 500, 'ASM', device=device)
...
recovery_slm_field = calculate_slm_input(n=25, dx=dx, d0=d0, lambda_=lambda_, device=device)
# recovery_slm_field = torch.load('asm/asm_p/recovery_slm_field.pth')

# Calculate hologram
phase = torch.zeros(1, 1, 690, 690).to(device)
...
best_loss = 10.0
gt_sensor = cv2.imread('asm/1.png', cv2.IMREAD_GRAYSCALE) / 255
gt_sensor = cv2.resize(gt_sensor,(1600,1600), interpolation=cv2.INTER_CUBIC)
gt_sensor = pad_array(gt_sensor, 2000, 2000, 0)
...
for i in pbar:
    ...
    slm_field = torch.exp(1j * phase_ex) * recovery_slm_field
...
# plt.imsave('asm/asm_p/load_slm_p.png', best_phase[0][0], cmap='gray')
