...
for sample in tqdm(loader):
    ...
    with torch.no_grad():
        ...
        order_nums = [3, 3, 3, 3, 3]
        filters = [3.0, 3.0, 3.0, 3.0, 3.0]
        pupils = [2.0*mm, 3.0*mm, 4.0*mm, 5.0*mm, 6.0*mm]

        mse_list = utils.evaluate_phase(slm_phases, scene_amp, prop_dist,
            pitch, wavelengths, roi_res, slm_res, order_nums, filters,
            pupils=pupils, masks_c=None, loss=nn.MSELoss(),
            ...
            eyepiece=opt.eyepiece*mm)

        for k in range(len(mse_list)):
            sio.savemat(os.path.join(opt.output_path,
                f'{sum_id}_filter{filters[k]}_order{order_nums[k]}_pupil{pupils[k]}_Img{idx}.mat'),
                {f'mse': mse_list[k]})
